...
@ModuleInfo(name = "DamageIndicators", category = ModuleCategory.MODS)
class DamageIndicators : Module() {
    ...
    private val colourValue = ListValue("ColourType", arrayOf("Red", "Custom", "Rainbow"), "None")
    ...
    //private val colorRainbowValue = colourValue.get().equals("Rainbow")
        ...
        synchronized(particles) {
            for(entity in mc.theWorld.loadedEntityList) {
                if(entity is EntityLivingBase && EntityUtils.isSelected(entity,true) && mc.thePlayer.getDistanceToEntity(entity) < 5) {
                    ...
                    val prefix = if (!colourValue.get().equals("Rainbow")) (if(lastHealth>entity.health){"❤"}else{return}) else (if(lastHealth>entity.health){"-"}else{return})
    ...
    fun onRender3d(event: Render3DEvent) {
        synchronized(particles) {
            ...
            for (particle in particles) {
                ...
                val doge = if (colourValue.get().equals("Red") ) "§c" else ""
                ...
                mc.fontRendererObj.drawString(doge + particle.str, (-(mc.fontRendererObj.getStringWidth(particle.str) / 2)), (-(mc.fontRendererObj.FONT_HEIGHT - 1)), (if (colourValue.get().equals("Rainbow")) ColorUtils.rainbowWithAlpha(colorAlphaValue.get()) else Color(colorRedValue.get(), colorGreenValue.get(), colorBlueValue.get(), colorAlphaValue.get())).rgb)
