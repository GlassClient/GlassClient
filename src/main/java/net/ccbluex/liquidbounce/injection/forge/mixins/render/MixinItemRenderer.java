...
public abstract class MixinItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        ...
        if (this.itemToRender != null) {
            ...
            final boolean displayBlocking = LiquidBounce.moduleManager.getModule(Animations.class).getFakeBlocking();
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch((animations.getAnythingBlockValue().get()) ? EnumAction.BLOCK : this.itemToRender.getItemUseAction()) {
                    case NONE:
    ...
    private void renderFireInFirstPerson(float p_color_0_, float p_color_1_, float p_color_2_, float p_color_3_) {
        ...
        if(p_color_3_ != 1.0f && ClearView.getState()){
            GlStateManager.color(p_color_0_, p_color_1_, p_color_2_, ClearView.getFireEffectValue().get());
        }else{
